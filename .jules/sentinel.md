...
## YYYY-MM-DD - [Fix Command Injection in external plugin dependency checks]
**Vulnerability:** The memory plugin setup routine (`_install_dependencies`) in `hermes_cli/memory_setup.py` executed untrusted `check` commands from `plugin.yaml` using `subprocess.run(..., shell=True)`. This could allow arbitrary code execution when processing external configurations.
**Learning:** External configurations, such as dependency checks defined in `plugin.yaml`, are untrusted and must never be evaluated by a shell. Additionally, catching exceptions with `subprocess.run` silently ignores command failure unless `check=True` is provided.
**Prevention:** Always tokenize external commands with `shlex.split()`, set `shell=False`, and provide `check=True` if intending to catch execution errors with `try/except`.
