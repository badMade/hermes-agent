...
## YYYY-MM-DD - [Fix Command Injection in Plugin Dependency Checks]
**Vulnerability:** The `hermes_cli/memory_setup.py` script was executing external plugin dependency checks (`check_cmd`) using `subprocess.run(..., shell=True)` without any tokenization or escaping. This allows a malicious plugin configuration (which provides `check_cmd`) to execute arbitrary shell commands (Remote Code Execution / Command Injection).
**Learning:** External configurations, such as plugin metadata defining commands, must be treated as untrusted input. Executing them with `shell=True` creates a direct command injection vector. Even if it breaks legitimate shell features like pipes, project security constraints mandate tokenizing with `shlex.split()` and setting `shell=False`.
**Prevention:** Always use `shlex.split()` and `shell=False` when executing untrusted external commands from plugin configurations. Additionally, ensure `check=True` is used inside `try...except Exception` blocks catching command failures, otherwise exceptions won't be raised for non-zero exit codes.
