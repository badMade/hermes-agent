...
**Vulnerability:** The memory provider setup (`_install_dependencies` in `hermes_cli/memory_setup.py`) used `subprocess.run(check_cmd, shell=True)` to verify external dependencies.
**Learning:** Using `shell=True` for untrusted dependency checks provided in plugin metadata exposes the setup process to arbitrary command injection.
**Prevention:** Always tokenize command strings for untrusted external configurations using `shlex.split()` and use `shell=False` (along with `check=True` to properly catch execution failures).
