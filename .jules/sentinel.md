...
## 2025-02-28 - [Fix Command Injection in Memory Provider Dependency Check]
**Vulnerability:** The memory provider setup CLI (`hermes_cli/memory_setup.py`) executed dependency verification commands from external, untrusted `plugin.yaml` files (`check_cmd`) using `subprocess.run(..., shell=True)`.
**Learning:** Configurations loaded from external files like `plugin.yaml` should be treated as untrusted input. Executing them with `shell=True` allows arbitrary command injection.
**Prevention:** Always tokenize command strings using `shlex.split()` and set `shell=False` when executing external configuration values without shell dependencies.
