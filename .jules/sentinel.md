...
## YYYY-MM-DD - [Fix Command Injection in Memory Plugin Dependency Check]
**Vulnerability:** The memory plugin installation process in `hermes_cli/memory_setup.py` executed untrusted external dependency check commands (`check_cmd` from `plugin.yaml`) using `subprocess.run(..., shell=True)`.
**Learning:** Using `shell=True` with untrusted configuration values allows arbitrary command injection, potentially leading to Remote Code Execution (RCE) if a malicious plugin configuration is loaded.
**Prevention:** Always parse untrusted command strings using `shlex.split()` and execute them with `shell=False` to prevent shell injection vulnerabilities.
