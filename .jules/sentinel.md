...
## 2025-02-28 - [Fix Command Injection in memory setup dependency installation]
**Vulnerability:** The `hermes_cli/memory_setup.py` module executed untrusted shell commands from an external plugin's `plugin.yaml` using `subprocess.run(check_cmd, shell=True)` during dependency installation.
**Learning:** External configurations that dictate command executions are inherently untrusted. Passing arbitrary configuration values directly to `shell=True` allows for command injection (RCE), overriding any cross-platform compatibility arguments.
**Prevention:** Always treat external command configurations as untrusted, tokenizing them via `shlex.split()` and executing with `shell=False` to prevent command injection, regardless of potential shell feature regressions.
