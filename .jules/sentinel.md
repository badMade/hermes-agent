...
## YYYY-MM-DD - [Fix Command Injection in memory plugin dependencies check]
**Vulnerability:** `hermes_cli/memory_setup.py` evaluated the external dependency `check_cmd` from a plugin's yaml configuration using `subprocess.run(check_cmd, shell=True)`.
**Learning:** External plugin configuration files are untrusted external input and can contain malicious arbitrary shell commands that get executed via `shell=True`.
**Prevention:** Treat any external configuration inputs as untrusted, tokenize them with `shlex.split()`, and set `shell=False` to prevent command injection.
