...
## 2024-05-25 - [Fix Command Injection in memory_setup.py]
**Vulnerability:** The `_install_dependencies` function in `hermes_cli/memory_setup.py` executed `plugin.yaml` dependency check commands (`check_cmd`) using `subprocess.run(..., shell=True)` without environment sanitization.
**Learning:** This exposes the application to command injection if `plugin.yaml` is untrusted or tampered with, and leaks main process environment variables to the child process.
**Prevention:** For untrusted configuration commands, avoid `shell=True` by tokenizing with `shlex.split()`, and always sanitize the environment using `_sanitize_subprocess_env`.
