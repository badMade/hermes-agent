...
## YYYY-MM-DD - Command Injection in memory plugin dependencies
**Vulnerability:** `subprocess.run(check_cmd, shell=True)` was used to execute dependency check commands sourced from untrusted external `plugin.yaml` files.
**Learning:** When executing configurations supplied by external plugins, they must be treated as untrusted input. Additionally, when placing `subprocess.run` inside a `try...except Exception:` block meant to catch command failures, `check=True` must be added, otherwise `subprocess.run` will silently return a `CompletedProcess` on a non-zero exit code and bypass the fallback logic.
**Prevention:** Always use `shlex.split()` and `shell=False` for external commands, and remember to include `check=True` when expecting to catch command failures as exceptions.
