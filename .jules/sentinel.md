...
## 2025-02-28 - [Fix Webhook Secret Validation Bypass with Defaults]
**Vulnerability:** The `_looks_unresolved_secret` function in `gateway/platforms/webhook.py` used a regex `^\$\{[A-Za-z_][A-Za-z0-9_]*\}$` that only matched simple environment variable placeholders like `${WEBHOOK_SECRET}`. It failed to match placeholders with default values like `${WEBHOOK_SECRET:-default}`.
**Learning:** Placeholders with default values are common in configuration files, and if the environment variable is not set, the literal string `${WEBHOOK_SECRET:-default}` could be passed as the secret. An attacker who knows or guesses the configuration structure could forge a valid HMAC signature using this literal string, bypassing authentication.
**Prevention:** Ensure regexes used for identifying unresolved environment placeholders account for common bash/shell parameter expansion syntax, specifically default values like `:-default`. Always use `r"^\$\{[A-Za-z_][A-Za-z0-9_]*(?::-[^}]*)?\}$"` for full coverage.
