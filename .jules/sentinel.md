...
## 2026-08-02 - [Fix Command Injection in Plugin Dependency Checks]
**Vulnerability:** The `_install_dependencies` function in `hermes_cli/memory_setup.py` executed external dependency check commands sourced from `plugin.yaml` files using `subprocess.run(check_cmd, shell=True)`.
**Learning:** Sourcing commands from plugin configuration files and executing them with `shell=True` allows for arbitrary command injection if the configuration file is malicious or compromised.
**Prevention:** Always tokenize command strings sourced from configurations using `shlex.split()` and use `shell=False` to prevent command injection.
