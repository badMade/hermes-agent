...
## 2026-05-23 - Security Enhancement: Avoid shell=True in Quick Commands
**Vulnerability:** Command injection via `subprocess.run(shell=True)` in `cli.py` and `tui_gateway/server.py` for user-configured quick commands and arbitrary shell execution.
**Learning:** Passing user-defined command strings directly to `subprocess.run` with `shell=True` is a classic command injection vector. It bypasses any application-level checks and allows arbitrary command execution. Also, while `detect_dangerous_command` exists, it is not a sandbox and should not be relied upon to prevent all malicious shell strings.
**Prevention:** Replace `shell=True` with `shell=False` (or omit it) and parse the command string into a list of arguments using `shlex.split()` before passing it to `subprocess.run()`. Always combine with `_sanitize_subprocess_env` to prevent credential leakage to child processes.
