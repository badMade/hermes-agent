...
## YYYY-MM-DD - [Fix Command Injection in memory provider plugin dependency check]
**Vulnerability:** The `hermes_cli/memory_setup.py` script was executing external dependency checks from untrusted memory provider `plugin.yaml` files using `subprocess.run(..., shell=True)` without tokenizing the command string.
**Learning:** Untrusted external configuration commands executed with `shell=True` allow arbitrary command injection (RCE).
**Prevention:** Always tokenize command strings using `shlex.split()` and use `shell=False` for external commands, and include `check=True` in try-except blocks to catch process errors.
