...
## 2025-02-27 - [Fix Command Injection Vulnerability in Dependency Checks]
**Vulnerability:** The memory setup tool (`hermes_cli/memory_setup.py`) executed the `check_cmd` specified in external/untrusted `plugin.yaml` configurations using `subprocess.run(check_cmd, shell=True)`.
**Learning:** Using `shell=True` to execute dependency verification commands sourced from external configurations allows arbitrary command injection. A malicious plugin repository could specify arbitrary shell commands in its `plugin.yaml` and compromise the system when `hermes memory setup` is run. The `install_cmd` is also loaded but only printed to the user for manual execution, so it is not a direct command execution vulnerability itself.
**Prevention:** Always use `shlex.split()` and set `shell=False` when executing commands sourced from external configurations or untrusted input strings.
