...
## 2024-05-28 - [Fix Command Injection in Plugin Dependency Checks]
**Vulnerability:** The memory plugin setup routine (`hermes_cli/memory_setup.py`) executed `check_cmd` from untrusted external `plugin.yaml` configurations using `subprocess.run(..., shell=True)`.
**Learning:** External configurations, such as plugin manifests, should be treated as untrusted input. Using `shell=True` to execute commands defined in these files allows arbitrary command injection.
**Prevention:** Always parse and execute commands from external configurations using `shlex.split()` and set `shell=False`, and use `check=True` when catching failures.
