...
## 2025-02-28 - [Fix Webhook Secret Placeholder Validation]
**Vulnerability:** The webhook platform adapter (`gateway/platforms/webhook.py`) checked if a configured secret was an unresolved placeholder using a regular expression (`^\$\{[A-Za-z_][A-Za-z0-9_]*\}$`). However, this regex failed to match placeholders that included default values, such as `${WEBHOOK_SECRET:-default}`. This could result in a literal string containing a placeholder being treated as a real HMAC secret, weakening authentication since an attacker might guess the placeholder name.
**Learning:** When validating whether environment variable placeholders remain unresolved in configuration values, the validation logic must account for various placeholder syntaxes, including default value fallback syntax (`:-`).
**Prevention:** Use a more comprehensive regex (e.g., `^\$\{[A-Za-z_][A-Za-z0-9_]*(?::-[^}]*)?\}$`) with start `^` and end `$` anchors that correctly identifies placeholders with or without default values to ensure unresolved secrets are securely rejected.
