...
**Prevention:** Replace `shell=True` with `shell=False` (or omit it) and parse the command string into a list of arguments using `shlex.split()` before passing it to `subprocess.run()`.
