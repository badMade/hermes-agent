...
## YYYY-MM-DD - [Fix Command Injection in Memory Provider Plugin External Dependencies]
**Vulnerability:** The `hermes_cli/memory_setup.py` script executed untrusted `check_cmd` commands from memory provider `plugin.yaml` files using `subprocess.run(..., shell=True)`.
**Learning:** Untrusted external configurations, such as plugin dependency checks, should never be executed with `shell=True` as this allows arbitrary command injection.
**Prevention:** Always tokenize command strings using `shlex.split()` and set `shell=False` when executing commands from external configurations.
