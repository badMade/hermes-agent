...
## 2025-02-28 - [Properly Fix Zip Slip Vulnerability with Windows Path Awareness]
**Vulnerability:** A previous patch to fix a Zip Slip vulnerability replaced `Path(name).parts` with `name.split("/")` to avoid `NameError`. However, `name.split("/")` only splits on forward slashes, leaving backslashes unparsed. This allowed Windows-style path traversals (e.g., `..\..\Windows\System32\cmd.exe`) to bypass the `..` check and successfully exploit the Zip Slip vulnerability on Windows systems.
**Learning:** Python's native `pathlib.Path` handles cross-platform path parsing (including backslashes on Windows), but when string manipulation is used for traversal checks instead, it must explicitly account for backslashes.
**Prevention:** When validating archive members for path traversal vulnerabilities, explicitly check for both forward and backward slashes, or normalize them before splitting. Use `".." in member.name.replace("\\", "/").split("/")` and `name.startswith("\\")` instead of just `.split("/")`.
