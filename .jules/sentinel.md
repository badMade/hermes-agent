...
## 2025-02-27 - [Fix Command Injection in memory provider plugin installs]
**Vulnerability:** The memory provider setup mechanism `_install_dependencies` inside `hermes_cli/memory_setup.py` reads `external_dependencies` from an untrusted `plugin.yaml` file and executes the `check` command using `subprocess.run(..., shell=True)` without tokenizing the arguments or sanitizing the environment.
**Learning:** External plugins configurations should be treated as untrusted data. Executing their raw commands via `shell=True` allows trivial command injection vulnerabilities on the host machine.
**Prevention:** Always tokenize such plugin commands using `shlex.split()` and execute them with `shell=False`, and sanitize the environment using `_sanitize_subprocess_env` to avoid credential leakage.
