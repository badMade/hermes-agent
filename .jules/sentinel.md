...
## YYYY-MM-DD - [Remove Untrusted Third-Party GitHub Action]
**Vulnerability:** A third-party GitHub Action (`badMade/agy-pr-review`) was present in `.github/workflows/pr-review.yml` and configured to receive the sensitive `ANTIGRAVITY_API_KEY` (or `GEMINI_API_KEY`), leading to potential credential exfiltration and supply-chain attacks.
**Learning:** Third-party CI/CD actions that have access to sensitive secrets represent a critical supply chain risk. An untrusted or compromised action can easily steal secrets from the environment.
**Prevention:** Never pass sensitive secrets to unauthorized or untrusted third-party GitHub Actions. Completely delete malicious workflow files rather than attempting to fix them.
