...
## YYYY-MM-DD - Bulk Task Promotion Optimization in kanban_db.py
**Learning:** The `recompute_ready` function in `hermes_cli/kanban_db.py` was suffering from an N+1 query bottleneck, iterating over all `todo` tasks to check their parents' statuses individually. By manually expanding `_append_event` into a raw `INSERT INTO` to attempt batch insertion, domain logic and safe abstractions were incorrectly bypassed, leading to regression risk.
**Action:** When optimizing N+1 query loops using a single `EXISTS`/`NOT EXISTS` SQL query and `executemany`, only batch the low-level updates. Continue calling established internal wrapper functions like `_append_event` individually in a loop over the targeted dataset to preserve side effects, encapsulation, and domain logic.
