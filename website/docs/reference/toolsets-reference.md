...
|---------|-------|---------|
| `browser` | `browser_back`, `browser_click`, `browser_console`, `browser_dialog`, `browser_get_images`, `browser_navigate`, `browser_press`, `browser_scroll`, `browser_snapshot`, `browser_type`, `browser_vision`, `web_search` | Core browser automation. Includes `web_search` as a fallback for quick lookups. `browser_dialog` is gated at runtime — registered only when a CDP endpoint is reachable at session start. It works together with the `pending_dialogs` and `frame_tree` fields that `browser_snapshot` adds when a CDP supervisor is attached. |
| `browser-cdp` | `browser_cdp` | Opt-in restricted raw Chrome DevTools Protocol access. Cookie access/mutation and CDP navigation methods are blocked; use `browser_navigate` for navigation policy enforcement. |
...
| `hermes-webhook` | No tools by default. Webhook payloads are untrusted input even when provider-signed; explicitly opt in to required toolsets per deployment. |
