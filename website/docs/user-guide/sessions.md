...
| WhatsApp DM | `agent:main:whatsapp:dm:<canonical_identifier>` | One session per DM user. LID/phone aliases collapse only when bridge mapping proves they refer to the same account; otherwise the JID namespace is preserved to prevent cross-user collisions. |
