...
This skill sets up a scheduled workflow that monitors GitHub repository Pull Request comments for the exact phrase `@jules`. A matching comment is only a review request when it was posted by a trusted repository participant and the PR branch is from the same repository. Authorized requests are reviewed using the existing `github-code-review` skill's static-review methodology, and the PR is labeled only after the review is safely completed.

## How It Works

1. **Trigger**: A scheduled job runs periodically (e.g., via cron).
2. **Scan**: It queries GitHub for open PR comments mentioning `@jules`.
3. **Authorize**: It confirms at least one matching comment was made by an `OWNER`, `MEMBER`, or `COLLABORATOR`.
...
7. **Mark Done**: Once the review completes, it adds the `jules-reviewed` label and posts the review comment.
...
# Search for open PRs with matching comments, excluding PRs already reviewed.
...
while read -r PR_NUMBER; do
  ...
  # Require at least one matching trigger comment from a trusted repository participant.
  TRUSTED_COMMENT_COUNT=$(gh api --paginate "repos/$REPO/issues/$PR_NUMBER/comments" \
    --jq '.[] | select((.body // "") | contains("@jules")) | select(.author_association == "OWNER" or .author_association == "MEMBER" or .author_association == "COLLABORATOR") | .id' \
    | wc -l | tr -d ' ')
