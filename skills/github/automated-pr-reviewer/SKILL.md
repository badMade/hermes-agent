...
2. **Scan**: It queries GitHub for open PR comments mentioning `@jules code review` (all pages).
...
- Only treat a comment as a trigger when it contains the exact phrase `@jules code review`.
...
while read -r PR_NUMBER; do
  ...
  # Require at least one genuine trigger comment ("@jules code review")
  # from a trusted repository participant.
  TRUSTED_COMMENT_COUNT=$(gh api --paginate "repos/$REPO/issues/$PR_NUMBER/comments" \
    --jq '.[] | select((.body // "") | test("@jules code review", "i")) | select(.author_association == "OWNER" or .author_association == "MEMBER" or .author_association == "COLLABORATOR") | .id' \
    | wc -l | tr -d ' ')
