...
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `hermes-{profile}` isolates memory per active Hermes profile. Placeholder values are collision-resistant escaped before substitution, so punctuation-bearing IDs like email or Matrix users remain isolated. Empty placeholders collapse cleanly (e.g. `hermes-{user}` with no user becomes `hermes`). |
...
| `retain_tags` | — | Default tags applied to retained memories |
