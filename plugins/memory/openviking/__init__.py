...
class OpenVikingMemoryProvider(MemoryProvider):
    ...
    def _tool_add_resource(self, args: dict) -> str:
        ...
        if _is_remote_resource_source(url):
            payload["path"] = url
        elif _is_windows_absolute_path(url) or parsed_url.scheme == "file":
            return tool_error(local_path_error)
        elif parsed_url.scheme:
            # Non-remote scheme (ftp://, data://, mailto://, etc.)
            return tool_error(
                "Invalid resource URL. Provide a public http(s), git, or ssh URL."
            )
        else:
            # No scheme: treat as local path (absolute, relative, or bare name)
            return tool_error(local_path_error)
