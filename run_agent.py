...
class AIAgent:
    ...
    def __init__(
        ...
        try:
            ...
            if (
                _ll_enabled
                and self._use_prompt_caching
                and self._supports_long_lived_anthropic_cache()
            ):
                self._use_long_lived_prefix_cache = True
        except Exception as e:
            logger.warning("Failed to load prompt_caching config", exc_info=True)
        ...
        self.session_log_file = (
            self.logs_dir
            / f"session_{_safe_session_path_component(self.session_id)}.json"
        )
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            dump_file = (
                self.logs_dir
                / f"request_dump_{_safe_session_path_component(self.session_id)}_{timestamp}.json"
            )
    ...
    def run_conversation(
        ...
        while (
            ...
            if self._use_long_lived_prefix_cache:
                _sys_parts = self._build_system_prompt_parts(
                ...
                if self.ephemeral_system_prompt:
                    _sys_blocks.append({
                        "type": "text",
                        "text": self.ephemeral_system_prompt,
                    })
                if _sys_blocks:
                    api_messages = [
                        {"role": "system", "content": _sys_blocks}
                    ] + api_messages
            else:
