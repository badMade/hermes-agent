...
class WebhookAdapter(BasePlatformAdapter):
    ...
    def _validate_signature(
        ...
        # Reject unresolved env-var placeholders like ${WEBHOOK_SECRET} to avoid
        # accepting signatures computed with a literal placeholder string as key.
        if re.search(r'\$\{[^}]+\}', secret):
            logger.warning(
                "[webhook] Secret contains unresolved placeholder; rejecting signature"
            )
            return False
