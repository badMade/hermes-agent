...
class WebhookAdapter(BasePlatformAdapter):
    ...
    def _validate_signature(
        ...
        if _looks_unresolved_secret(secret) or not has_usable_secret(secret, min_length=1):
            logger.warning("[webhook] Rejecting request: configured secret is a placeholder")
            return False
