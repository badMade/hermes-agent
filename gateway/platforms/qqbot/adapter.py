...
class QQAdapter(BasePlatformAdapter):
    ...
    async def _handle_c2c_message(
        ...
        authorized_for_attachments = True
        if self.gateway_runner and hasattr(self.gateway_runner, "_is_user_authorized"):
            authorized_for_attachments = bool(self.gateway_runner._is_user_authorized(source))

        text = content
        attachments_raw = d.get("attachments") if authorized_for_attachments else None
        logger.info(
