...
def test_acp_image_block_rejects_local_file_uri(tmp_path):
    attached = tmp_path / "private.png"
    attached.write_bytes(_ONE_PX_PNG)
...
def test_acp_image_block_allows_http_urls():
    content = _content_blocks_to_openai_user_content([
        ImageContentBlock(
            type="image",
            data="",
...
def test_acp_image_block_rejects_oversized_inline_data():
    oversized_b64 = base64.b64encode(b"x" * (512 * 1024 + 1)).decode("ascii")

    content = _content_blocks_to_openai_user_content([
        TextContentBlock(type="text", text="too large"),
        ImageContentBlock(type="image", data=oversized_b64, mimeType="image/png"),
    ])

    assert content == "too large"
