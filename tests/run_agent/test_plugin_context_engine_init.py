...
def test_general_plugin_engine_is_cloned_per_agent():
    """General plugin context engines should not share per-session state."""
    from hermes_cli.plugins import PluginContext, PluginManager, PluginManifest

    prototype = _StubEngine()
    prototype.session_marker = "prototype"
    manager = PluginManager()
    PluginContext(
        PluginManifest(name="test-engine"),
        manager,
    ).register_context_engine(prototype)
    ...
    with (
        ...
        patch(
            ...
            side_effect=manager.create_context_engine,
        ),
        patch("agent.model_metadata.get_model_context_length", return_value=131_072),
        patch("run_agent.get_tool_definitions", return_value=[]),
        patch("run_agent.check_toolset_requirements", return_value={}),
    ...
    assert first.context_compressor is not second.context_compressor
    assert first.context_compressor is not prototype
    assert second.context_compressor is not prototype
    first.context_compressor.session_marker = "session-a"
    assert second.context_compressor.session_marker == "prototype"
    assert prototype.session_marker == "prototype"
