...
class TestIsWsl:
    """Tests for is_wsl() — Windows Subsystem for Linux detection."""

    def _reset_cache(self, monkeypatch):
        """Reset the cached detection result before each test."""
        monkeypatch.setattr(hermes_constants, "_wsl_detected", None)

    def test_detects_wsl(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            "builtins.open",
            lambda p, *a, **kw: (
                _real_open(str(version_file), *a, **kw)
                if p == "/proc/version"
                else _real_open(p, *a, **kw)
            ),
        )
    ...
    def test_negative_case(self, monkeypatch, tmp_path):
        ...
        version_file = tmp_path / "version"
        version_file.write_text(
            "Linux version 5.4.0-74-generic (buildd@lcy01-amd64-010)\n"
        )
    ...
    def test_exception_handling(self, monkeypatch):
        """Returns False when reading /proc/version raises an exception."""
        import builtins
    ...
    def test_caches_result(self, monkeypatch):
        ...
        # Even if reading /proc/version would fail or return False, cached value wins
        import builtins

        def _mock_open(*args, **kwargs):
            raise AssertionError("Should not read /proc/version when cached")
