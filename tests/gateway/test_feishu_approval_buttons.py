...
class TestCardActionCallbackResponse:
    ...
    def test_update_prompt_honors_per_chat_group_allowlist(self, _patch_callback_card_types):
        adapter = _make_adapter()
        adapter._loop = MagicMock()
        adapter._loop.is_closed = MagicMock(return_value=False)
        adapter._group_rules["oc_secure"] = feishu_module.FeishuGroupRule(
            policy="allowlist",
            allowlist={"ou_authorized"},
        )
        adapter._update_prompt_state[1] = {
            "session_key": "sess-up-1",
        ...
        data = _make_card_action_data(
            {"hermes_update_prompt_action": "n", "update_prompt_id": 1},
            chat_id="oc_secure",
            open_id="ou_intruder",
        )
    ...
    def test_update_prompt_honors_gateway_authorization(self, _patch_callback_card_types):
        adapter = _make_adapter()
        adapter._loop = MagicMock()
        adapter._loop.is_closed = MagicMock(return_value=False)
        ...
        assert response.card is None
        mock_submit.assert_not_called()
        adapter.gateway_runner._is_user_authorized.assert_called_once()

    def test_update_prompt_rejects_click_from_wrong_chat(self, _patch_callback_card_types):
        adapter = _make_adapter()
        adapter._loop = MagicMock()
        adapter._loop.is_closed = MagicMock(return_value=False)
        adapter._update_prompt_state[1] = {
