...
class TestDefaultInteractionDispatch:
    ...
    async def test_approval_click_rejects_unauthorized_operator(self, monkeypatch):
        ...
        event = parse_interaction_event({
            ...
            "group_member_openid": "evil-member",
            "data": {"resolved": {"button_data": "approve:sess-abc:allow-once"}},
        })
    ...
    @pytest.mark.asyncio
    async def test_update_prompt_click_rejects_unauthorized_operator(
    ...
    @pytest.mark.asyncio
    async def test_approval_click_authorizes_group_operator(self, monkeypatch):
        adapter = self._make_adapter()
        seen_sources = []
        adapter.set_interaction_authorizer(
            lambda source: seen_sources.append(source) or True
        )
        resolve_calls = []
        ...
        assert resolve_calls == [("sess-abc", "once", False)]
        assert seen_sources[0].chat_id == "group-1"
        assert seen_sources[0].user_id == "member-1"
