...
class TestWecomCallbackAuthorization:
    def _runner(self, approved=()):
        ...
        class PairingStore:
            ...
            def is_approved(self, platform_name, user_id):
                self.calls.append((platform_name, user_id))
                return (platform_name, user_id) in self.approved

        runner = object.__new__(GatewayRunner)
        runner.pairing_store = PairingStore(approved)
        return runner

    def _source(self, corp_id, user_id):
        from gateway.config import Platform
        from gateway.session import SessionSource

        return SessionSource(
            platform=Platform.WECOM_CALLBACK,
            chat_id=f"{corp_id}:{user_id}",
            chat_type="dm",
            user_id=user_id,
            user_name=user_id,
        )

    def test_allowlist_requires_corp_scoped_user_id(self, monkeypatch):
        monkeypatch.setenv("WECOM_CALLBACK_ALLOWED_USERS", "corpA:alice")
        monkeypatch.delenv("WECOM_CALLBACK_ALLOW_ALL_USERS", raising=False)
        monkeypatch.delenv("GATEWAY_ALLOWED_USERS", raising=False)
        monkeypatch.delenv("GATEWAY_ALLOW_ALL_USERS", raising=False)
        runner = self._runner()

        assert runner._is_user_authorized(self._source("corpA", "alice")) is True
        assert runner._is_user_authorized(self._source("corpB", "alice")) is False

    def test_bare_allowlist_does_not_cross_authorize_corps(self, monkeypatch):
        monkeypatch.setenv("WECOM_CALLBACK_ALLOWED_USERS", "alice")
        monkeypatch.delenv("WECOM_CALLBACK_ALLOW_ALL_USERS", raising=False)
        monkeypatch.delenv("GATEWAY_ALLOWED_USERS", raising=False)
        monkeypatch.delenv("GATEWAY_ALLOW_ALL_USERS", raising=False)
        runner = self._runner()
    ...
    def test_pairing_store_uses_corp_scoped_user_id(self, monkeypatch):
        monkeypatch.delenv("WECOM_CALLBACK_ALLOWED_USERS", raising=False)
        monkeypatch.delenv("WECOM_CALLBACK_ALLOW_ALL_USERS", raising=False)
        monkeypatch.delenv("GATEWAY_ALLOWED_USERS", raising=False)
        monkeypatch.delenv("GATEWAY_ALLOW_ALL_USERS", raising=False)
        runner = self._runner({("wecom_callback", "corpA:alice")})

        assert runner._is_user_authorized(self._source("corpA", "alice")) is True
        assert runner._is_user_authorized(self._source("corpB", "alice")) is False
        assert ("wecom_callback", "corpB:alice") in runner.pairing_store.calls
