...
class TestAgentCacheIdleResume:
    ...
    def test_cleanup_session_tool_resources_uses_session_id(self, monkeypatch):
        """Fallback hard cleanup removes tool state after cache-only eviction."""
        from gateway.run import GatewayRunner
        ...
        monkeypatch.setattr(
            ...
            lambda task_id: calls.append(("terminal", task_id)),
        )
        monkeypatch.setattr(
            "tools.browser_tool.cleanup_browser",
            lambda task_id: calls.append(("browser", task_id)),
        )
        runner._cleanup_stale_auxiliary_clients = lambda: calls.append(("aux", ""))

        runner._cleanup_session_tool_resources("sess-orphaned")

        assert ("process", "sess-orphaned") in calls
        assert ("terminal", "sess-orphaned") in calls
        assert ("browser", "sess-orphaned") in calls
        assert ("aux", "") in calls

    @pytest.mark.asyncio
    async def test_reset_hard_cleans_soft_evicted_session_by_session_id(self):
        """/new must clean session tool state even after cache eviction lost the agent."""
        from collections import OrderedDict
        ...
        old_entry = SessionEntry(
            ...
            session_id="old-soft-evicted-session",
        ...
        runner._agent_cache = OrderedDict()  # old agent was already soft-evicted
        runner._agent_cache_lock = threading.Lock()
        runner._running_agents = {}
        ...
        runner._cleanup_session_tool_resources = cleaned_session_ids.append

        event = MessageEvent(text="/new", source=source, message_id="m1")
        await runner._handle_reset_command(event)

        assert cleaned_session_ids == ["old-soft-evicted-session"]
