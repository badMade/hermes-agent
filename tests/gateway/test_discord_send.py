...
@pytest.mark.asyncio
async def test_send_animation_blocks_unsafe_redirect_target(monkeypatch):
    """Redirect targets must be SSRF-checked before Discord attachment upload."""
    adapter = DiscordAdapter(PlatformConfig(enabled=True, token="***"))
    public_url = "http://example.com/redirect.gif"
    internal_url = "http://127.0.0.1/secret"
    sent_msg = SimpleNamespace(id=9876)
    ...
    class FakeResponse:
        ...
        async def read(self):  # pragma: no cover - must not read redirected internals
            raise AssertionError("unsafe redirect response must not be read")
    ...
    fake_aiohttp = SimpleNamespace(ClientSession=FakeSession, ClientTimeout=FakeTimeout)
    monkeypatch.setitem(sys.modules, "aiohttp", fake_aiohttp)
    monkeypatch.setattr(
        "gateway.platforms.discord.is_safe_url",
        lambda url: url == public_url,
    )

    result = await adapter.send_animation("555", public_url, caption="gif")

    assert result.success is True
    channel.send.assert_awaited_once()
    send_kwargs = channel.send.await_args.kwargs
    assert send_kwargs["content"] == f"gif\n{public_url}"
