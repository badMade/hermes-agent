...
class TestHandleResumeCommand:
    ...
    @pytest.mark.asyncio
    async def test_list_named_sessions_scopes_to_gateway_user(self, tmp_path):
        """Gateway /resume must not list titled sessions owned by other users."""
        from hermes_state import SessionDB
        db = SessionDB(db_path=tmp_path / "state.db")
        db.create_session("victim_session", "telegram", user_id="victim")
        db.append_message("victim_session", "user", "private first prompt")
        db.set_session_title("victim_session", "Victim Work")
        db.create_session("attacker_session", "telegram", user_id="12345")

        event = _make_event(text="/resume")
        runner = _make_runner(
            session_db=db, current_session_id="attacker_session", event=event
        )
        result = await runner._handle_resume_command(event)

        assert "Victim Work" not in result
        assert "private first prompt" not in result
    ...
    @pytest.mark.asyncio
    async def test_resume_by_name_scopes_to_gateway_user(self, tmp_path):
        """Gateway /resume must not switch into another user's titled session."""
        from hermes_state import SessionDB
        db = SessionDB(db_path=tmp_path / "state.db")
        db.create_session("victim_session", "telegram", user_id="victim")
        db.set_session_title("victim_session", "Victim Work")
        ...
        event = _make_event(text="/resume Victim Work")
        runner = _make_runner(
            session_db=db, current_session_id="attacker_session", event=event
        )
        result = await runner._handle_resume_command(event)
