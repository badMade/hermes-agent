...
Covers:
- DDGSSearchProvider.is_configured() — uses package metadata without importing ddgs
...
def _configured_ddgs_provider(monkeypatch):
    """Return DDGSSearchProvider with metadata availability stubbed on."""
    from tools.web_providers import ddgs as ddgs_provider

    monkeypatch.setattr(ddgs_provider, "ddgs_package_available", lambda: True)
    return ddgs_provider.DDGSSearchProvider
...
class TestDDGSProviderIsConfigured:
    def test_configured_when_distribution_available(self, monkeypatch):
        from tools.web_providers import ddgs as ddgs_provider

        monkeypatch.setattr(ddgs_provider, "ddgs_package_available", lambda: True)
        assert ddgs_provider.DDGSSearchProvider().is_configured() is True

    def test_not_configured_when_package_missing(self, monkeypatch):
        from tools.web_providers import ddgs as ddgs_provider

        monkeypatch.setattr(ddgs_provider, "ddgs_package_available", lambda: False)
        assert ddgs_provider.DDGSSearchProvider().is_configured() is False

    def test_availability_does_not_import_shadowed_local_module(
        self, tmp_path, monkeypatch
    ):
        marker = tmp_path / "imported"
        (tmp_path / "ddgs.py").write_text(
...
class TestDDGSProviderSearch:
    def test_happy_path_normalizes_results(self, monkeypatch):
        ...
        DDGSSearchProvider = _configured_ddgs_provider(monkeypatch)
    ...
    def test_missing_package_returns_failure(self, monkeypatch):
        ...
        monkeypatch.setattr(ddgs_provider, "ddgs_package_available", lambda: False)
        result = ddgs_provider.DDGSSearchProvider().search("q", limit=5)
...
class TestDDGSSearchOnlyErrors:
    def test_web_extract_returns_search_only_error(self, monkeypatch):
        ...
        monkeypatch.setattr(web_tools, "_ddgs_package_available", lambda: True)
        monkeypatch.setattr(web_tools, "_is_tool_gateway_ready", lambda: False)
        # Allow example.com to pass SSRF checks so backend check is reached
