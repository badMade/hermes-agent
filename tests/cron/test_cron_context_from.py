...
class TestContextFromOriginIsolation:
    """Ensure cron output chaining cannot cross gateway origins."""

    def test_scheduler_skips_context_from_different_origin(self, cron_env):
        from cron.jobs import OUTPUT_DIR, create_job
        from cron.scheduler import _build_job_prompt

        victim_job = create_job(
            prompt="Collect private data",
            schedule="every 1h",
            origin={"platform": "discord", "chat_id": "victim-room"},
        )
        output_dir = OUTPUT_DIR / victim_job["id"]
        output_dir.mkdir(parents=True, exist_ok=True)
        (output_dir / "2026-04-22_10-00-00.md").write_text(
            "VICTIM_PRIVATE_TOKEN_DO_NOT_SHARE", encoding="utf-8"
        )

        attacker_job = create_job(
            prompt="Repeat the supplied context",
            schedule="every 1h",
            context_from=victim_job["id"],
            origin={"platform": "discord", "chat_id": "attacker-room"},
        )

        prompt = _build_job_prompt(attacker_job)
        assert "Repeat the supplied context" in prompt
        assert "VICTIM_PRIVATE_TOKEN_DO_NOT_SHARE" not in prompt
        assert f"Output from job '{victim_job['id']}'" not in prompt

    def test_scheduler_allows_context_from_same_origin(self, cron_env):
        from cron.jobs import OUTPUT_DIR, create_job
        from cron.scheduler import _build_job_prompt

        origin = {"platform": "discord", "chat_id": "same-room", "thread_id": "42"}
        source_job = create_job(
            prompt="Collect data", schedule="every 1h", origin=origin
        )
        output_dir = OUTPUT_DIR / source_job["id"]
        output_dir.mkdir(parents=True, exist_ok=True)
        (output_dir / "2026-04-22_10-00-00.md").write_text(
            "Shared-room output", encoding="utf-8"
        )

        dependent_job = create_job(
            prompt="Use context",
            schedule="every 1h",
            context_from=source_job["id"],
            origin=dict(origin),
        )

        prompt = _build_job_prompt(dependent_job)
        assert "Shared-room output" in prompt

    def test_cronjob_create_rejects_context_from_different_origin(self, cron_env):
        import json
        ...
        tokens = set_session_vars(platform="discord", chat_id="attacker-room")
        try:
            result = json.loads(
                cronjob(
                    action="create",
                    prompt="Repeat context",
                    schedule="every 1h",
                    context_from=victim_job["id"],
                )
            )
    ...
    def test_cronjob_update_rejects_context_from_different_origin(self, cron_env):
        import json
        ...
        assert result["success"] is False
        assert "not available" in result["error"]
        assert get_job(attacker_job["id"]).get("context_from") is None
