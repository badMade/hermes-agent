...
class TestRuntimeLoadConfigEnvGate:
    """``hermes_cli.config.load_config()`` must share the same isolation gate.

    Runtime components such as MCP and plugin discovery use this loader during
    startup, so ``--ignore-user-config`` must prevent those components from
    seeing user-configured behavior.
    """

    def test_runtime_load_config_skips_user_config_when_flag_set(self, tmp_path, monkeypatch):
        config_yaml = textwrap.dedent(
            """
            model:
              default: user-model-should-be-ignored
            plugins:
              enabled:
                - user-plugin-should-be-ignored
            mcp_servers:
              user-server-should-be-ignored:
                command: /bin/echo
                args: [ignored]
            """
        ).lstrip()
        ...
        monkeypatch.setattr(config_mod, "get_config_path", lambda: tmp_path / "config.yaml")
        cfg = config_mod.load_config()

        model_cfg = cfg.get("model", {})
        model_default = model_cfg.get("default") if isinstance(model_cfg, dict) else model_cfg
        assert model_default != "user-model-should-be-ignored"
        assert "user-plugin-should-be-ignored" not in cfg.get("plugins", {}).get("enabled", [])
        assert "user-server-should-be-ignored" not in cfg.get("mcp_servers", {})

    def test_runtime_load_config_loads_user_config_when_flag_unset(self, tmp_path, monkeypatch):
        config_yaml = textwrap.dedent(
            """
            model:
              default: user-model-is-loaded
            plugins:
              enabled:
                - user-plugin-is-loaded
            mcp_servers:
              user-server-is-loaded:
                command: /bin/echo
            """
        ).lstrip()
        (tmp_path / "config.yaml").write_text(config_yaml)
        ...
        assert cfg.get("model", {}).get("default") == "user-model-is-loaded"
        assert "user-plugin-is-loaded" in cfg.get("plugins", {}).get("enabled", [])
        assert "user-server-is-loaded" in cfg.get("mcp_servers", {})
