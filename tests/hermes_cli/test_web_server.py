...
class TestPluginAPIAuth:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        Use ``/api/plugins/kanban/board`` — a stable,
        ...
        ws.app.router.routes = [
            ...
            if not (hasattr(r, "path") and "kanban/board" in r.path)
        ]

        # Add the route
        router_to_add = router
        ws.app.include_router(router, prefix="/api/plugins/kanban")

        # Now move it to the front
        routes = ws.app.router.routes
        new_route = routes[-1]
        routes.pop()
