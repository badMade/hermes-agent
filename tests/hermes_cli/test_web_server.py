...
class TestPluginAPIAuth:
    ...
    def test_plugin_route_allows_auth(self):
        ...
        Use ``/api/plugins/hermes-achievements/scan-status`` to verify the middleware.
        Without a valid token the middleware should 401 before the handler.
        With a valid token the request passes the middleware and hits the router,
        which returns 404 (Not Found) because the real plugin is not mounted in
        this isolated test environment. Asserting 404 instead of 401 proves the
        ...
        # With auth: middleware allows it through to the router (which 404s).
