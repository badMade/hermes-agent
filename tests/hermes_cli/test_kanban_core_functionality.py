...
# ---------------------------------------------------------------------------
# Kanban profile authorization
# ---------------------------------------------------------------------------

def test_named_profile_create_cannot_target_sibling_profile_by_default(
    tmp_path, monkeypatch, capsys
):
    from hermes_cli import kanban as kb_cli

    root = tmp_path / ".hermes"
    low = root / "profiles" / "low"
    high = root / "profiles" / "high"
    low.mkdir(parents=True)
    high.mkdir(parents=True)
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("HERMES_HOME", str(low))
    monkeypatch.setattr(
        kb_cli, "_assignee_policy_config",
        lambda: {"allowed_assignees": []},
    )
    kb.init_db()

    ns = _make_create_ns(title="deny", assignee="high", skills=["privileged"])
    assert kb_cli._cmd_create(ns) == 2
    captured = capsys.readouterr()
    assert "not allowed to assign" in captured.err
    with kb.connect() as conn:
        assert kb.list_tasks(conn) == []


def test_named_profile_create_allows_self_assignment(tmp_path, monkeypatch, capsys):
    from hermes_cli import kanban as kb_cli
    ...
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("HERMES_HOME", str(low))
    monkeypatch.setattr(kb_cli, "_check_dispatcher_presence", lambda: (True, ""))
    monkeypatch.setattr(
        kb_cli, "_assignee_policy_config",
        lambda: {"allowed_assignees": []},
    )
    ...
    ns = _make_create_ns(title="self", assignee="low")
    assert kb_cli._cmd_create(ns) == 0
    assert "Created" in capsys.readouterr().out


def test_named_profile_create_allows_configured_delegate(
    tmp_path, monkeypatch, capsys
):
    from hermes_cli import kanban as kb_cli
    ...
    monkeypatch.setattr(
        kb_cli, "_assignee_policy_config",
        lambda: {"allowed_assignees": ["high"]},
    )
    kb.init_db()

    ns = _make_create_ns(title="delegate", assignee="high")
    assert kb_cli._cmd_create(ns) == 0
    assert "Created" in capsys.readouterr().out


def test_named_profile_log_cannot_read_sibling_task_by_default(
    tmp_path, monkeypatch, capsys
):
    from hermes_cli import kanban as kb_cli
    ...
    kb.init_db()
    with kb.connect() as conn:
        task_id = kb.create_task(conn, title="secret", assignee="high")
    log_path = kb.worker_log_path(task_id)
    log_path.parent.mkdir(parents=True, exist_ok=True)
    log_path.write_text("privileged output\n", encoding="utf-8")

    monkeypatch.setenv("HERMES_HOME", str(low))
    ns = argparse.Namespace(task_id=task_id, tail=None)
    assert kb_cli._cmd_log(ns) == 2
    captured = capsys.readouterr()
    assert "not allowed to read worker logs" in captured.err
    assert "privileged output" not in captured.out
