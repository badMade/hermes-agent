...
@pytest.mark.asyncio
async def test_gateway_notify_subscribe_ignores_spoofed_notifier_profile(kanban_home):
    """Gateway /kanban notify-subscribe must bind ownership to active profile."""
    from gateway.run import GatewayRunner
    from gateway.config import Platform
    ...
    runner = object.__new__(GatewayRunner)
    runner._kanban_notifier_profile = "victim-profile"
    runner._active_profile_name = lambda: "victim-profile"
    source = SimpleNamespace(
        platform=Platform.TELEGRAM,
        chat_id="chat1",
        thread_id="th1",
    ...
    event = SimpleNamespace(
        text=(
            f"/kanban notify-subscribe {task_id} --platform telegram "
            "--chat-id attacker-chat --notifier-profile attacker-profile"
        ),
        source=source,
    )
    ...
    try:
        ...
        conn.close()
    assert len(subs) == 1
    assert subs[0].get("notifier_profile", "") == "victim-profile"
    assert subs[0].get("chat_id", "") == "attacker-chat"
    assert subs[0].get("platform", "") == "telegram"
