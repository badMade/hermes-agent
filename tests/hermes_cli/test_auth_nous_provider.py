...
def test_access_token_refresh_preserves_custom_ca_bundle(tmp_path, monkeypatch):
    import ssl
    ...
    monkeypatch.setattr(
        ...
        lambda **kwargs: cafiles.append(kwargs.get("cafile")) or mock_ctx,
    )
    ...
    assert auth_mod.resolve_nous_access_token() == "access-1"

    state_after_refresh = auth_mod.get_provider_auth_state("nous")
    assert state_after_refresh is not None
    assert state_after_refresh["tls"] == {
        "insecure": False,
        "ca_bundle": str(ca_file),
    }
    assert cafiles == [str(ca_file)]


def test_runtime_credentials_preserve_custom_ca_bundle(tmp_path, monkeypatch):
    import ssl
    from hermes_cli import auth as auth_mod
    ...
    def _fake_mint_agent_key(*, client, portal_base_url, access_token, min_ttl_seconds):
        return _mint_payload(api_key="agent-key-1")
    ...
    assert creds["api_key"] == "agent-key-1"
    state_after_mint = auth_mod.get_provider_auth_state("nous")
    assert state_after_mint is not None
    assert state_after_mint["tls"] == {
        "insecure": False,
        "ca_bundle": str(ca_file),
    }
    assert cafiles == [str(ca_file)]
