...
class TestMemoryProviderABC:
    ...
    def test_is_available_abstract(self):
        """Subclass failing to implement is_available raises TypeError."""

        class MissingIsAvailableProvider(MemoryProvider):
            @property
            def name(self) -> str:
                return "missing"
        ...
        with pytest.raises(
            TypeError,
            match="Can't instantiate abstract class MissingIsAvailableProvider",
        ):
            MissingIsAvailableProvider()
    ...
    def test_default_methods_and_hooks_coverage(self):
        """Verify default returns for optional abstract class hooks and methods."""
        ...
        with pytest.raises(
            NotImplementedError, match="Provider minimal does not handle tool unknown"
        ):
