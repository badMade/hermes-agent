...
class TestMemoryManager:
    ...
    def test_initialize_all(self):
        """initialize_all calls initialize on all providers and injects hermes_home."""
        ...
        assert p1._init_kwargs["session_id"] == "test_session"
        assert p1._init_kwargs["hermes_home"] == expected_home

        assert p2.initialized is True
        assert p2._init_kwargs["session_id"] == "test_session"
        assert p2._init_kwargs["hermes_home"] == expected_home

    def test_initialize_all_with_explicit_hermes_home(self):
        """initialize_all respects explicitly provided hermes_home."""
        mgr = MemoryManager()
        p1 = FakeMemoryProvider("builtin")
        mgr.add_provider(p1)

        mgr.initialize_all("test_session", hermes_home="/custom/path")

        assert p1.initialized is True
        assert p1._init_kwargs["session_id"] == "test_session"
        assert p1._init_kwargs["hermes_home"] == "/custom/path"

    def test_initialize_all_tolerates_failure(self):
        """If one provider's initialize fails, others still initialize."""
        mgr = MemoryManager()

        p1 = FakeMemoryProvider("builtin")
        p1.initialize = MagicMock(side_effect=RuntimeError("init boom"))
        p2 = FakeMemoryProvider("external")
        ...
        # p1 failed (mock replaced the actual method so initialized attribute won't be set by it)
        # but p2 still initialized
        assert p2.initialized is True
        assert p2._init_kwargs["session_id"] == "test_session"
