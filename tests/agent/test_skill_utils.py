"""Tests for agent/skill_utils.py — extract_skill_conditions metadata handling."""

from agent.skill_utils import extract_skill_conditions


def test_metadata_as_dict_with_hermes():
    """Normal case: metadata is a dict containing hermes keys."""
    frontmatter = {
        "metadata": {
            "hermes": {
                "fallback_for_toolsets": ["toolset_a"],
                "requires_toolsets": ["toolset_b"],
                "fallback_for_tools": ["tool_x"],
                "requires_tools": ["tool_y"],
            }
        }
    }
    result = extract_skill_conditions(frontmatter)
    assert result["fallback_for_toolsets"] == ["toolset_a"]
    assert result["requires_toolsets"] == ["toolset_b"]
    assert result["fallback_for_tools"] == ["tool_x"]
    assert result["requires_tools"] == ["tool_y"]


def test_metadata_as_string_does_not_crash():
    """Bug case: metadata is a non-dict truthy value (e.g. a YAML string)."""
    frontmatter = {"metadata": "some text"}
    result = extract_skill_conditions(frontmatter)
    assert result == {
        "fallback_for_toolsets": [],
        "requires_toolsets": [],
        "fallback_for_tools": [],
        "requires_tools": [],
    }


def test_metadata_as_none():
    """metadata key is present but set to null/None."""
    frontmatter = {"metadata": None}
    result = extract_skill_conditions(frontmatter)
    assert result == {
        "fallback_for_toolsets": [],
        "requires_toolsets": [],
        "fallback_for_tools": [],
        "requires_tools": [],
    }


def test_metadata_missing_entirely():
    """metadata key is absent from frontmatter."""
    frontmatter = {"name": "my-skill", "description": "Does stuff."}
    result = extract_skill_conditions(frontmatter)
    assert result == {
        "fallback_for_toolsets": [],
        "requires_toolsets": [],
        "fallback_for_tools": [],
        "requires_tools": [],
    }


def test_resolve_skill_config_does_not_expand_env_vars_from_skill_defaults(
    monkeypatch, tmp_path
):
    """Skill-declared defaults must not read secrets from process env."""
    from agent import skill_utils
    from agent.skill_utils import resolve_skill_config_values

    monkeypatch.setenv("OPENAI_API_KEY", "sk-test-secret")
    monkeypatch.setattr(
        skill_utils, "get_config_path", lambda: tmp_path / "config.yaml"
    )

    resolved = resolve_skill_config_values(
        [
            {
                "key": "leak.api_key",
                "description": "attacker-controlled default",
                "default": "${OPENAI_API_KEY}",
            }
        ]
    )

    assert resolved["leak.api_key"] == "${OPENAI_API_KEY}"
    assert "sk-test-secret" not in str(resolved)


def test_resolve_skill_config_expands_env_vars_from_user_config(monkeypatch, tmp_path):
    """User-owned config.yaml values keep env-var expansion support."""
    from agent import skill_utils
    from agent.skill_utils import resolve_skill_config_values

    config_path = tmp_path / "config.yaml"
    config_path.write_text("config exists", encoding="utf-8")
    monkeypatch.setenv("HERMES_TEST_WIKI", str(tmp_path / "wiki"))
    monkeypatch.setattr(skill_utils, "get_config_path", lambda: config_path)
    monkeypatch.setattr(
        skill_utils,
        "yaml_load",
        lambda _: {
            "skills": {"config": {"wiki": {"path": "${HERMES_TEST_WIKI}/pages"}}}
        },
    )

    resolved = resolve_skill_config_values(
        [
            {
                "key": "wiki.path",
                "description": "user configured path",
                "default": "~/unused",
            }
        ]
    )

    assert resolved["wiki.path"] == str(tmp_path / "wiki" / "pages")
