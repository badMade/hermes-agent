...
def test_process_manager_rejects_cross_session_status_transcript_say_and_stop():
    from plugins.google_meet import process_manager as pm

    out_dir = Path(os.environ["HERMES_HOME"]) / "workspace" / "meetings" / "abc-defg-hij"
    out_dir.mkdir(parents=True)
    (out_dir / "transcript.txt").write_text("Alice: secret\n", encoding="utf-8")
    pm._write_active({
        "pid": 0,
    ...
    for res in (
        pm.status(session_id="attacker-session"),
        pm.status(),
        pm.transcript(session_id="attacker-session"),
        pm.enqueue_say("hello", session_id="attacker-session"),
        pm.stop(session_id="attacker-session"),
    ):
        assert res["ok"] is False
        assert "different Hermes session" in res["reason"]

    assert pm._read_active() is not None
    assert not (out_dir / "say_queue.jsonl").exists()


def test_meet_tool_handlers_forward_session_id_to_process_manager():
    ...
    with patch("plugins.google_meet.tools.check_meet_requirements", return_value=True), \
         patch("plugins.google_meet.tools.pm.start", return_value={"ok": True}) as start_mock:
        json.loads(handle_meet_join(
            {"url": "https://meet.google.com/abc-defg-hij"},
            session_id="session-a",
        ))
    assert start_mock.call_args.kwargs["session_id"] == "session-a"

    with patch("plugins.google_meet.tools.pm.status", return_value={"ok": True}) as status_mock:
        json.loads(handle_meet_status({}, session_id="session-a"))
    assert status_mock.call_args.kwargs["session_id"] == "session-a"

    with patch("plugins.google_meet.tools.pm.transcript", return_value={"ok": True}) as transcript_mock:
        json.loads(handle_meet_transcript({}, session_id="session-a"))
    assert transcript_mock.call_args.kwargs["session_id"] == "session-a"

    with patch("plugins.google_meet.tools.pm.stop", return_value={"ok": True}) as stop_mock:
        json.loads(handle_meet_leave({}, session_id="session-a"))
    assert stop_mock.call_args.kwargs["session_id"] == "session-a"

    with patch("plugins.google_meet.tools.pm.enqueue_say", return_value={"ok": True}) as say_mock:
        json.loads(handle_meet_say({"text": "hello"}, session_id="session-a"))
    assert say_mock.call_args.kwargs["session_id"] == "session-a"
